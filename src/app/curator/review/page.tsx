...
import { seedArchiveItems, seedSources, seedStories } from "@/lib/data/seed";
...
export default async function CuratorReviewPage({ searchParams }: PageProps<"/curator/review">) {
  ...
  const queue = [
    ...seedArchiveItems.filter((item) => item.reviewStatus !== "approved").map((item) => ({ id: item.id, kind: "archive item", title: item.title, status: item.reviewStatus })),
    ...seedStories.filter((story) => story.reviewStatus !== "approved").map((story) => ({ id: story.id, kind: "story", title: story.title[locale], status: story.reviewStatus })),
    ...seedSources.filter((source) => source.approvalStatus !== "approved").map((source) => ({ id: source.id, kind: "source", title: source.title, status: source.approvalStatus })),
  ];
