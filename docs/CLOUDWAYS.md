...
7. Create the first administrator only once. Supply `BOOTSTRAP_ADMIN_EMAIL`, `BOOTSTRAP_ADMIN_DISPLAY_NAME`, and `BOOTSTRAP_ADMIN_PASSWORD` transiently; `BOOTSTRAP_ADMIN_MFA_REFERENCE` is additionally required when `STAFF_MFA_REQUIRED=true`. Set `BOOTSTRAP_CONFIRM=CREATE_INITIAL_ADMIN`, run `npm run auth:bootstrap-admin`, and remove every bootstrap variable immediately afterward. The command refuses to run when an administrator already exists and does not print the password or hash.
