...
These fixtures are intentionally rejected in production. MFA is temporarily disabled when `STAFF_MFA_REQUIRED` is unset or `false`; set it to `true` to restore the development code `000000` and the database verification hook. The database identity adapter stores versioned scrypt password hashes and reloads active roles and accepted family membership on every request. Production self-service password recovery uses short-lived, single-use emailed links, revokes every existing session after a successful change, and never bypasses staff MFA. Museum-controlled MFA enrollment and invitation administration remain production requirements.
...
Set `AUTH_PROVIDER=database`, `DEV_AUTH_ENABLED=false`, and independent random `AUTH_SESSION_SECRET` and `PASSWORD_RESET_TOKEN_KEY` values of at least 32 bytes. Family accounts authenticate with their stored scrypt hash and exactly one accepted, non-revoked family membership. Password recovery also requires the complete pinned Elastic Email SMTP configuration documented below; the application stores only keyed token digests and puts the raw token in a URL fragment that the browser immediately removes. The staging preview currently uses `STAFF_MFA_REQUIRED=false`, so admin and curator accounts use password authentication while role checks remain active. Before production, set `STAFF_MFA_REQUIRED=true`; staff accounts must then have `mfa_required=true`, an opaque `mfa_provider_reference`, and an HTTPS verification service configured with `MFA_PROVIDER=webhook` and `MFA_VERIFY_URL`.
